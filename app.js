...
const currdate=date.format(nDate,"ddd, MMM DD YYYY");
const time=date.format(nDate,"hh:mm A");
app.get('/',(req,res)=>{
...
app.post('/checkin',async(req,res)=>{
    ...
    const ifuseralreadycheckedin=await visitor.exists({$and:[{username:{$eq:username}},{status:{$eq:'checked-in'}}]});
        ...
        const checkedin=await date.format(nDate,"ddd, MMM DD YYYY | hh:mm A");
...
app.post('/checkout',async(req,res)=>{
     try{
       ...
       const checkedout=await date.format(nDate,"ddd, MMM DD YYYY | hh:mm A");
       ...
       const currdate=await date.format(nDate,"ddd, MMM DD YYYY");
       const time=await date.format(nDate,"hh:mm A");
       const from='nirmitsawhney01@gmail.com';
